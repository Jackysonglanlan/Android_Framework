...
public class XMLDecompressor {
  ...
  private Closure<String> rowHandler;
  
  public void setRowHandler(Closure<String> rowHandler) {
    ...
    while (off < xml.length) {
      ...
      if (tag0 == startTag) { // XML START TAG
        ...
        for (int ii = 0; ii < numbAttrs; ii++) {
          ...
          String attrValue = attrValueSi != -1 ? compXmlString(xml, sitOff, stOff, attrValueSi) : "resourceID 0x"
        ...
        System.out.println("  Unrecognized tag code '" + Integer.toHexString(tag0) + "' at offset " + off);
        break;
      }
  ...
  private void prtIndent(int indent, String str) {
    rowHandler.doWith(spaces.substring(0, Math.min(indent * 2, spaces.length())) + str);
  }
  ...
  private int LEW(byte[] arr, int off) {
    return arr[off + 3] << 24 & 0xff000000 | arr[off + 2] << 16 & 0xff0000 | arr[off + 1] << 8 & 0xff00 | arr[off]
        & 0xFF;
  } // end of LEW
