...
package jacky.song.android.util.collection;

import jacky.song.android.util.lang.ArrayUtils;
import jacky.song.android.util.lang.ObjectUtils;
...
public abstract class CollectionUtils {
	 ...
	 * @return <code>true</code> if found, <code>false</code> else
	 */
	public static <T> boolean containsInstance(Collection<T> collection, T element) {
		if (collection != null) {
	 ...
	 */
	public static <T> boolean containsAny(Collection<T> source, Collection<? extends T> candidates) {
	 ...
	 */
	public static <T> T findFirstMatch(Collection<T> source, Collection<? extends T> candidates) {
	 ...
	 */
	public static <T> T findValueOfType(Collection<T> collection, Class<?> type) throws IllegalArgumentException {
		if (isEmpty(collection)) { return null; }
		Class<?> typeToUse = (type != null ? type : Object.class);
		T value = null;
		for (T e : collection) {
			if (typeToUse.isInstance(e)) {
				if (value != null) { throw new IllegalArgumentException("More than one value of type [" + typeToUse.getName()
						+ "] found"); }
				value = e;
			}
		}
	...
	/**
	 * Find a value of one of the given types in the given Collection: searching the Collection for a
	 * value of the first type, then searching for a value of the second type, etc.
	 * 
	 * @param collection
	 *          the collection to search
	 * @param types
	 ...
	 *           if more than one value of the given type found
	 */
	public static <T> T findValueOfType(Collection<T> collection, Class<?>[] types) throws IllegalArgumentException {
		if (isEmpty(collection) || ObjectUtils.isEmpty(types)) { return null; }
		for (int i = 0; i < types.length; i++) {
			T value = findValueOfType(collection, types[i]);
	...
	public static <T> boolean hasUniqueObject(Collection<T> collection) {
		...
		for (T e : collection) {
			...
			else if (candidate != e) { return false; }
		}
		return true;
	}

	public static <T> T[] toArray(final Iterator<T> itor, final Class<? extends T[]> newType) {
		List<T> list = toList(itor);
		return ArrayUtils.toArray(list, newType);
	}

	public static <T> List<T> toList(Iterator<T> itor) {
		return toList(itor, 12);
	}

	public static <T> List<T> toList(Iterator<T> itor, int estimatedSize) {
	...
	public static void addToList(Iterator itor, List list) {
		while (itor.hasNext()) {
			list.add(itor.next());
		}
	}
